...
public class BonusEncoger extends GImage {
	 ...
	 */
	public BonusEncoger(double posX, double posY, double ancho, double alto, String bonus) {
		super(bonus);
		setSize(ancho, alto);
	...
	private boolean chequeaColision(double posX, double posY, Arkanoid arkanoid){
		...
		if(aux instanceof Cursor){//Si el bonus se choca con el cursor
			...
			((Cursor) aux).setSize(80, aux.getHeight());
			((Cursor) aux).setFillColor(Color.GRAY);
			setLocation(1000,0);
